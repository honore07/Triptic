...
function WeatherIcon({ code }: { code: number }) {
  ...
  if (code >= 95) return <CloudLightning size={18} className="text-storm" aria-hidden="true" />;
  if ((code >= 71 && code <= 77) || code === 85 || code === 86)
    return <CloudSnow size={18} className={cls} aria-hidden="true" />;
  if ((code >= 51 && code <= 67) || (code >= 80 && code <= 82))
    return <CloudRain size={18} className={cls} aria-hidden="true" />;
  if (code === 45 || code === 48) return <CloudFog size={18} className={cls} aria-hidden="true" />;
  if (code === 3) return <Cloud size={18} className={cls} aria-hidden="true" />;
  if (code === 2) return <CloudSun size={18} className={cls} aria-hidden="true" />;
  return <Sun size={18} className="text-gold-deep" aria-hidden="true" />;
}
...
export function WeatherStrip({ trip, plan }: Props) {
  ...
  if (gated) {
    return (
      <p className="flex items-center gap-2 rounded-xl border border-mist bg-terrain px-4 py-3 text-sm text-ridge">
  ...
  return (
    <section aria-labelledby="weather-title" className="flex flex-col gap-2">
      <h2 id="weather-title" className="font-display text-xl font-bold text-trail">
        {t('weather.title')}
      </h2>
      ...
      <ol className="flex gap-2 overflow-x-auto pb-1">
        {days.map((d) => (
          <li
            key={d.day}
            className="flex min-w-[7.5rem] shrink-0 flex-col gap-1 rounded-xl border border-mist bg-snow px-3 py-2.5"
          >
            ...
            {d.forecast ? (
              <>
                <span className="flex items-center gap-2">
                  ...
                  <span className="font-mono text-sm text-trail">
      ...
      <p className="text-xs text-fog">{t('weather.source')}</p>
    </section>
  );
}
