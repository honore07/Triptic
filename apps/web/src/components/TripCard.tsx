...
export function TripCard({ trip, onChoose, index = 0 }: Props) {
  ...
  return (
    <article
      className="trip-card-enter group relative flex min-h-[420px] flex-col justify-end overflow-hidden rounded-trip bg-trail shadow-lg transition-transform duration-200 hover:-translate-y-1"
    ...
    >
      {trip.photo_url ? (
        ...
        <div className="absolute inset-0 bg-gradient-to-br from-ridge to-trail">
          <RoutePreview waypoints={trip.waypoints} className="h-full w-full opacity-40" stroke="#8AA0B8" />
        </div>
      )}
      <div
        className="absolute inset-0"
        style={{ background: 'linear-gradient(180deg, transparent 30%, rgba(13,27,42,0.88) 100%)' }}
        aria-hidden="true"
      />

      <div className="relative z-10 flex flex-col gap-3 p-5">
        <div className="flex items-center gap-2">
          <span className="rounded-badge bg-snow/15 px-2 py-0.5 text-xs font-medium text-snow backdrop-blur">
            {t(`mode.${trip.mode}`)}
          </span>
          <DifficultyBadge level={trip.difficulty} />
        </div>
        <h3 className="font-display text-xl font-bold leading-tight text-snow">{trip.title}</h3>
        <p className="line-clamp-2 text-sm text-mist">{trip.summary}</p>
        ...
        <button
          ...
          className="mt-1 min-h-11 w-full rounded-xl bg-summit px-4 py-2.5 text-sm font-semibold text-snow transition-colors hover:bg-summit/90"
        >
          {t('trips.choose')}
        </button>
      </div>
